...
class VideoProcessor:
    ...
    def get_video_info(self, video_path: Path) -> Dict[str, Any]:
        ...
        try:
            if MOVIEPY_AVAILABLE:
                ...
                duration = float(info['format']['duration']) if 'duration' in info['format'] else 0.0
                
                return {
                    'duration': duration,
                    'fps': eval(video_stream['r_frame_rate']) if video_stream and 'r_frame_rate' in video_stream else 30.0,
                    'size': (int(video_stream['width']), int(video_stream['height'])) if video_stream else (1920, 1080),
    ...
    def create_video_with_isolated_audio(self, 
                                       original_video: Path, 
                                       isolated_audio: Path, 
                                       output_video: Path) -> bool:
        """
