...
class ElevenLabsTTSProvider(SpeakerDiarizationProvider):
    ...
    def generate_tts_audio(
        self, text: str, voice_id: str, settings: Optional[Dict[str, Any]] = None
    ) -> bytes:
        """
    ...
    def calculate_voice_speed_profile(
        ...
        for i, text in enumerate(test_texts):
            try:
                ...
                if duration > 0:
                    ...
                    logger.debug(
                        "Test %d: %.2fs for %d chars (%d words)",
                        i + 1,
                        duration,
                        len(text),
                        len(text.split()),
        ...
        logger.info(
            "Voice speed profile for %s: %.1f chars/sec, %.1f words/sec",
            voice_id,
            chars_per_second,
            words_per_second,
        )
    ...
    def predict_tts_duration(self, text: str, voice_profile: Dict[str, float]) -> float:
        """
    ...
    def generate_adaptive_timeline(
        ...
        for speaker in speakers:
            ...
            if voice_id:
                logger.info(
                    "Profiling voice speed for speaker %s (voice: %s)",
                    speaker,
                    voice_id,
                )
    ...
    def generate_tracks_from_transcript(
        ...
        """
        logger.info(
            "Generating TTS tracks from %d segments (mode: %s)",
        ...
        if timing_mode == "adaptive":
            ...
            logger.info(
                "Timeline adjusted: %.1fs -> %.1fs",
                total_duration,
                adjusted_total_duration,
            )
        else:
    ...
    def _generate_segments_concurrent(
        ...
        """
        logger.info(
            "Generating TTS for %d segments with %d workers",
            len(transcript_segments),
            max_workers,
        )
        ...
        def generate_segment_audio(
            seg_index: int, segment: Dict[str, Any]
    ...
    def save_tracks_as_wav(
        self, tracks: Dict[str, AudioSegment], output_dir: str, base_filename: str
    ) -> List[str]:
        """
