...
class ElevenLabsSTTProvider(SpeakerDiarizationProvider):
    ...
    def transcribe_audio(
        ...
        try:
            ...
            logger.info(
                "Transcription completed: %d words, language: %s (%.1f%% confidence)",
    ...
    def convert_to_diarization_format(
        self, stt_result: Dict[str, Any], audio_duration: Optional[float] = None
        ...
        logger.info(
            "Converted to %d utterances from %d speakers",
            len(utterances),
            len(set(u["speaker"] for u in utterances)),
        )
    ...
    def save_results(
        ...
        # Save transcript
        transcript_file = (
            output_path / f"{base_filename}_elevenlabs_stt_transcript{debug_suffix}.txt"
        )
