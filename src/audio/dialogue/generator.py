...
class DialogueGenerator:
    ...
    def generate_dialogue_with_fallback(
        self, dialogue: DialogueBase, output_file: Path, use_fallback: bool = True
    ) -> bool:
        """
        ...
        try:
            ...
            for i, segment in enumerate(dialogue.segments):
                ...
                success = tts_provider.generate_speech(
                    text=segment.text, voice_id=segment.voice_id, output_file=temp_file
                )
    ...
    def estimate_generation_cost(self, dialogue: DialogueBase) -> Dict[str, Any]:
        ...
        unique_voices = len(
            set(segment.voice_id for segment in dialogue.segments if segment.voice_id)
        )
    ...
    def test_dialogue_api_availability(self) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/v1/text-to-dialogue", json=test_data, timeout=30
            )
    ...
    def preview_dialogue_generation(
        self, dialogue: DialogueBase, max_segments: int = 3
    ) -> Dict[str, Any]:
        """
