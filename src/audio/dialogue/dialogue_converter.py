...
class TranscriptToDialogueConverter:
    ...
    def analyze_dialogue_complexity(self, dialogue: DialogueBase) -> Dict[str, Any]:
        ...
        complex_punctuation = sum(1 for seg in segments if any(p in seg.text for p in ['...', '--', '()', '[]']))
        ...
        analysis = {
            ...
            'timing_stats': {
                ...
                'avg_segment_duration': round(avg_segment_duration, 2) if avg_segment_duration else None,
            ...
            'complexity_indicators': {
                ...
                'complexity_score': (long_segments + complex_punctuation) / total_segments if total_segments > 0 else 0
            },
            'recommendations': []
        }
        ...
        if long_segments > total_segments * 0.3:
            analysis['recommendations'].append("Consider breaking down long segments for better TTS flow")
        
        if short_segments > total_segments * 0.2:
            analysis['recommendations'].append("Consider combining very short segments to improve naturalness")
        
        if unique_speakers > 4:
            analysis['recommendations'].append("Large number of speakers may require careful voice selection")
