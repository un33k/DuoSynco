...
speaker_0 [13.8s - 17.2s]: Alright, we should probably get started then."""
    ...
    def run_comprehensive_test(
        self, transcript_file: Optional[Path] = None, language: str = "en"
    ) -> Dict[str, Any]:
        """
        ...
        if (
            results["transcript_parsing"]["parsing_success"]
            and results["voice_availability"]["voice_manager_available"]
        ):
        ...
        if parsing_ok and voices_ok and api_ok:
            ...
            results["recommendations"].append(
                "Basic transcript parsing works - check API key for voice features"
            )
        else:
            results["overall_status"] = "failed"
            results["recommendations"].append(
                "Multiple issues detected - check configuration and API access"
            )
