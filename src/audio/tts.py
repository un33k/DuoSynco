...
class TTSAudioGenerator:
    ...
    def __init__(self, provider: str = "elevenlabs", api_key: Optional[str] = None):
        """
    ...
    def generate_audio_tracks(
        ...
        saved_files = self.tts_provider.save_tracks_as_wav(
            tracks=tracks, output_dir=output_dir, base_filename=base_filename
        )
    ...
    def _validate_transcript_segments(self, segments: List[Dict[str, Any]]) -> None:
        ...
        for i, segment in enumerate(segments):
            ...
            if missing_keys:
                raise ValueError(f"Segment {i} missing required keys: {missing_keys}")
    ...
    def _calculate_stats(
        self, tracks: Dict[str, Any], transcript_segments: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def create_sample_transcript(
        speakers: List[str] = ["A", "B"], duration: float = 30.0
