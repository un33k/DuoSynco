...
class TranscriptEditor:
    ...
    def load_transcript(
        ...
        logger.info("Loaded transcript with %d utterances", 
    ...
    def replace_speaker_id(
        ...
        for i in range(start_idx, end_idx):
            if utterances[i].get('speaker') == old_speaker_id:
                # Record edit
                self._record_edit('replace_speaker', {
                    'utterance_index': i,
        ...
        logger.info("Replaced speaker ID '%s' -> '%s' in %d utterances", 
                   old_speaker_id, new_speaker_id, modified_count)
    ...
    def edit_utterance_text(
        ...
        logger.info("Edited utterance %d text: '%s...' -> '%s...'", 
                   utterance_index, old_text[:30], new_text[:30])
    ...
    def bulk_text_replace(
        ...
        for i, utterance in enumerate(utterances):
            ...
            if new_text != old_text:
                # Record edit
                self._record_edit('bulk_replace', {
                    'utterance_index': i,
        ...
        logger.info("Bulk replace '%s' -> '%s': %d utterances modified", 
                   search_pattern, replacement, modified_count)
    ...
    def merge_utterances(
        ...
        # Record edit
        self._record_edit('merge_utterances', {
        ...
        logger.info("Merged %d utterances (indices %d-%d) for speaker %s", 
                   end_index - start_index + 1, start_index, end_index, speaker)
    ...
    def split_utterance(
        ...
        # Record edit
        self._record_edit('split_utterance', {
            'utterance_index': utterance_index,
    ...
    def _adjust_utterance_timing(
        ...
        if old_length > 0:
            ...
            logger.debug("Adjusted timing for utterance %d: %.1fs -> %.1fs", 
                        utterance_index, original_duration, new_duration)