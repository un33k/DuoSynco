...
class SpeakerReplacer:
    ...
    def load_replacement_rules(self, rules_file: str) -> None:
        ...
        logger.info(
            "Loaded replacement rules: %d replacements, %d aliases, %d forbidden",
            len(self.replacement_rules),
            len(self.speaker_aliases),
            len(self.forbidden_replacements),
        )
    ...
    def add_replacement_rule(
        self, old_speaker: str, new_speaker: str, validate: bool = True
    ) -> bool:
        """
    ...
    def apply_replacement_rules(
        self, selective_rules: Optional[List[str]] = None
    ) -> Dict[str, int]:
        """
        ...
        if selective_rules:
            rules_to_apply = {
                k: v for k, v in self.replacement_rules.items() if k in selective_rules
            }
    ...
    def smart_speaker_replacement(
        ...
        for old_speaker, new_speaker in speaker_mapping.items():
            confidence = self._calculate_replacement_confidence(
                old_speaker, new_speaker, utterances
            )
        ...
        for old_speaker, new_speaker in speaker_mapping.items():
            ...
            if confidence >= confidence_threshold:
                ...
                logger.warning(
                    "Skipping replacement '%s' -> '%s' (confidence: %.2f < %.2f)",
                    old_speaker,
                    new_speaker,
                    confidence,
    ...
    def detect_speaker_patterns(self) -> Dict[str, Any]:
        ...
        patterns["case_inconsistencies"] = {
            k: v for k, v in patterns["case_inconsistencies"].items() if len(v) > 1
        }
        ...
        if patterns["generic_names"]:
            suggestions.append(
                f"Consider replacing {len(patterns['generic_names'])} generic speaker names with meaningful names"
            )

        if patterns["case_inconsistencies"]:
            suggestions.append(
                f"Normalize case inconsistencies for {len(patterns['case_inconsistencies'])} speaker name groups"
            )

        if patterns["similar_names"]:
            suggestions.append(
                f"Review {len(patterns['similar_names'])} pairs of similar speaker names for potential merging"
            )
    ...
    def _calculate_replacement_confidence(
        self, old_speaker: str, new_speaker: str, utterances: List[Dict[str, Any]]
    ) -> float:
