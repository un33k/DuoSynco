...
class EditWorkflow:
    ...
    def execute(
        ...
        try:
            ...
            stt_result = self._execute_stt_step(
                input_file, output_dir, speakers_expected, language, stt_quality, save_intermediate
            )
    ...
    def _execute_stt_step(
        ...
        logger.info("STT step completed: %d utterances, %d speakers", 
                   len(result['utterances']), len(result['speakers']))
    ...
    def _execute_edit_step(
        ...
        logger.info("Edit step completed: %d modifications, %d final speakers", 
                   len(edit_result['modifications']), edit_result['final_speakers'])
    ...
    def _execute_final_step(
        ...
        logger.info("Final step completed: %d speaker files, %.1f%% coverage", 
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        if 'final' in self.results:
            ...
            summary['steps_completed'].append({
                ...
                'total_speaker_duration': final_result.get('stats', {}).get('total_speaker_duration', 0)
            })
    ...
    def _save_workflow_metadata(self, output_dir: str, result: Dict[str, Any]) -> str:
        ...
        output_path = Path(output_dir)
        metadata_file = output_path / f"edit_workflow_metadata_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
