...
class DialogueWorkflow:
    ...
    def run_stt_to_dialogue_workflow(
        ...
        try:
            ...
            if use_character_profiles and character_profiles_file and character_profiles_file.exists():
                if self.config.verbose:
            ...
            if self.config.verbose:
                print(f"📊 Analyzed dialogue: {dialogue_stats['basic_stats']['total_segments']} segments, "
            ...
            if api_status.get('available', False):
                ...
                if self.config.verbose:
                    print(f"⚠️  Text to Dialogue API not available: {api_status.get('status', 'Unknown')}")
                results['errors'].append(f"API not available: {api_status.get('status', 'Unknown')}")
    ...
    def interactive_voice_assignment(
        ...
        for i, speaker in enumerate(speakers):
            ...
            while True:
                try:
                    choice = input(f"Select voice for {speaker} (1-{min(10, len(available_voices))}): ")
    ...
    def get_workflow_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'api_key_configured': hasattr(self.config, 'elevenlabs_api_key') and self.config.elevenlabs_api_key is not None
        }
    ...
    def create_sample_character_profiles(self, output_file: Path, language: str = "en") -> bool:
        ...
        try:
            ...
            for i, voice in enumerate(voices):
                ...
                character.speaking_style = ['calm', 'energetic', 'authoritative', 'dramatic'][i % 4]
